...
from checks import header_identity as dossier
...
class Identity(unittest.TestCase):
    ...
    def test_a_refused_candidate_found_by_convention_is_counted_only(self):
        result = check('identity', None, rejected=['dossier x names another page; not used'])
        self.assertEqual(result.findings, [])
        self.assertIn('refused=1', result.footer)

    def test_a_refused_candidate_named_by_option_fails(self):
        result = check('identity', None, how='option', rejected=['dossier x names another page; not used'])
        self.assertEqual([f.severity for f in result.findings], ['FAIL'])

    def test_no_dossier_at_all(self):
