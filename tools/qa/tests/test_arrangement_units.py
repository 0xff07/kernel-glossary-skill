...
class Behavior(unittest.TestCase):
    ...
    def test_the_recorded_block_map_is_compared_by_subsection(self):
        ...
        self.assertTrue(any('no dossier, not compared' in f.message for f in bare))
        ...
        found = list(check(page(now), TestInputs(dossier=dossier)))
        ...
        empty = list(check(page(now), TestInputs(dossier='## EVIDENCE\nnothing recorded\n')))
        self.assertTrue(any(f.severity == 'review' and 'no block map recorded' in f.message for f in empty))
        partial = list(check(page(now), TestInputs(dossier=f"## EVIDENCE\n| {lines['A']} A | P C P |\n")))
