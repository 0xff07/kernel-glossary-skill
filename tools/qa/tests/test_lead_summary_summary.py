"""Behavior and boundaries for lead summary summary."""
import unittest
from checks.lead_summary_summary import check
...
class Behavior(unittest.TestCase):
    ...
    def test_word_limit_and_sentence_inventory(self):
        text = skeleton(summary=' '.join(['word'] * 300))
        self.assertFalse([f for f in check(page(text), None) if f.severity == 'FAIL'])
        found = list(check(page(skeleton(summary=' '.join(['word'] * 301))), None))
    ...
    def test_structured_blocks(self):
        found = list(check(page(skeleton(summary=PROSE+'\n\n'+EXCERPT)), None))
