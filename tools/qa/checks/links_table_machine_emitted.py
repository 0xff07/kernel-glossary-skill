...
def emitted(page, inputs):
    ...
    findings = []
    if refused:
        listed = ', '.join((f'`{k}`' for k in refused[:REPORTED_REFUSALS])) + (f', and {len(refused) - REPORTED_REFUSALS} more' if len(refused) > REPORTED_REFUSALS else '')
        findings.append(Finding(None, 'review', f"{len(refused)} LINKS row(s) do not carry the table's {LINKS_COLUMNS} columns and were not read: {listed}; re-emit the table with kg table"))
    if refused_wholesale(rows, refused):
        ...
        if only_extractor or only_worksheet:
            findings.append(Finding(None, 'review', f"the LINKS table is not the page's: {len(only_extractor)} span(s) "
                                    f'without a row ({named(only_extractor)}) and {len(only_worksheet)} row(s) without a '
                                    f'span ({named(only_worksheet)}); run kg table, which rewrites the table and keeps the cells'))
