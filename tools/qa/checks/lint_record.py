...
def check(page, inputs):
    ...
    yield Finding(None, 'note', f'state {state}: {reason}', {'state': state, 'reason': reason, 'digest': inputs.page_digest, 'qa_digest': inputs.qa_digest, 'record_digest': record[1] if record else None, 'record_qa_digest': record[2] if record else None, 'self_run': bool(SELF_RUN.search(inputs.dossier_section('## LINT')))})
    if record and state != 'LINTED':
        yield Finding(None, 'review', reason + '; renew the record after the check pass')
    if SELF_RUN.search(inputs.dossier_section('## LINT')):
