...
Found by the `pm/port-power-management.md` writer, the first page written under PAGE-07 and the `docs/sound/` exemplars, and verified by the orchestrator against the tree.
...
- Pipeline: writer → orchestrator check per SKILL.md ("Modes") — the page is the writer's end to end (facts and prose, mechanical exit suite run, evidence persisted into the dossier); the orchestrator re-runs the checks per `guidelines/passes/03-check.md` and stamps WRITTEN → LINTED at batch checkpoints; a page's pipeline ends at LINTED.
