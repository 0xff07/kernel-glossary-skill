...
Campaign short name: `xhci`. Campaign file: `campaigns/xhci.md`; artifact directory: `progress/xhci/` (worksheets, lint/verify reports, completeness tables, and other agent intermediates land there; nothing outside it).
...
Documented tree: tag `v7.0`, commit `028ef9c96e96197026887c0f092424679298aae8` ("Linux 7.0"). semcode index complete at that commit (verified 2026-07-16). elixir.bootlin.com carries the tag (verified 2026-07-16); all Elixir links use `https://elixir.bootlin.com/linux/v7.0/source/...`. Subsystem Map entry xHCI (`guidelines/subsystems.md`): dir `xhci`, tag `usb` (secondary: `xhci`), kernel_paths `drivers/usb/host/xhci*` + `include/linux/usb/hcd.h`, spec "xHCI (eXtensible Host Controller Interface) Specification", section6_heading REGISTERS. The USB-core seam (`drivers/usb/core/`, `include/linux/usb.h`, `include/uapi/linux/usb/ch9.h`) is cited where xHCI meets it; no page documents USB-core internals beyond the seam.
...
Surfaced to the user for the settled rulings (kernel.md [links]); until it is folded in there, this amendment is the campaign's binding statement.
...
Pages carry MORE ASCII figures than the corpus has been averaging, wherever the subject has real structure to show. [coverage.constructs] already licensed this ("as many figures as the material earns"); the amendment is that writers should treat the figure budget as generous rather than minimal. Measured baseline for calibration: the four B9 pages carried 3 or 4 figures each, and the frozen samples run 1 to 3. Aim above that where the subject supports it, and expect register, layout and state-machine pages to carry the most.

Nothing about figure QUALITY relaxes, and two rules still outrank the larger budget:

- The four banned shapes of figures.md [shapes], which outrank the catalogs: a plain table redrawn in box characters, a call-flow graph whose every edge means "calls", a struct-member listing nothing exits, and plain text in a fence. Every figure passes the strip-the-labels test before it lands.
- The restraint principle of figures.md [drawing]: no figure for a plain call chain, a two-state toggle, or anything one declarative sentence conveys.

A page with eight figures of which two fail the strip test is worse than the same page with six. Figures are added by finding structure the prose cannot carry, never by drawing the same content twice or by promoting a semantics table into box characters, which is the first banned shape of [shapes.banned] and the standing hazard for register pages specifically.

The shapes that reliably earn their place on this campaign's remaining material: a register word partitioned into named bit ranges drawn to scale with L-connectors ([registers], calibrating against `docs/pci/protocol/tlp/msg/`); a link or endpoint state machine with what drives each edge; a timeline across separate actors for a sequence such as resume signalling or a reset; a field-to-field transform where one register's contents are reshaped into another structure; and a before-and-after pair around an operation that reshapes memory.
...
## Amendment 2026-09-02 — excerpt explanation ([excerpts.members-named]): the twelve pages of B9, B8 and B10 are repair candidates
...
Thirty-two of sixty-one excerpts (52%) have no member named beside them. This campaign's own July pages ran 24% on the same measurement, the sound corpus 15%, and the usb4 and pci corpora 13% and 16% (the per-era table is in `guidelines/reference/measured-criteria.md` (at commit 1bea4ac, before the compaction)). The doubling coincides with the 2026-08-29 to 2026-09-01 skill restructure and not with a model change: the enumeration ban's rule 3 as it read that week ("what its members share") was applied to excerpts instead of tables, the label-colon ban's text ([style.label-colon]) had removed the colon-terminated sentence that introduces an excerpt by naming what it shows, and the rewriter switchboard's humanizer entry barred the opening sentence from stating the heading's purpose. All three are corrected as of 2026-09-02 ([excerpts.members-named] added, with its generator; [style.run-on-enumeration]; the fence exemption of [style.label-colon]; `guidelines/reference/rewriters.md` (at commit 1bea4ac, before the compaction); QA item 8 in `guidelines/checking.md` section 2; step 7 in `guidelines/checking.md` section 7).

Binding from here:

- Every writer from the B7 remainder and B11 onward runs under [excerpts.members-named]: the paragraph beside a definition excerpt names each member the excerpt shows, says what it holds and which path writes or reads it, and a long definition is cut into several excerpts each explained where it stands. The QA steps's item 8 and the check pass's step 7 hold the gate; a zero-named excerpt is a defect, not a style note.
- The twelve pages above are repair candidates and are NOT overwritten silently; the overwrite guard applies. Repair is a fresh writer started from the page's worksheet plus this spec, briefed "work from the page's own field tables; explain each excerpt where you quote it", and the repair never shortens an excerpt to hide the members it would otherwise have to explain (eliding to the members the paragraph explains is the licensed cut). Order by the table's zero-named column, worst first, unless the user names a page.
- The corpus-level figures from the members generator are the calibration for the repairs: the sound corpus's 15% zero-named at a mean 0.37 named is the standard a repaired page meets, not this campaign's July 24%.
- [2026-09-11] Superseded on the operation only: there is no repair operation (`guidelines/campaign.md` [deriving]). A rewrite of one of the twelve pages removes it in a commit of its own and creates it again from the removed revision as prior material, with this amendment's findings as the derivation input; the calibration and the excerpt rules above stand.

## Amendment 2026-09-02 — two Area F anchors corrected at write time (pm/port-power-management)

Found by the `pm/port-power-management.md` writer, the first page written under [excerpts.members-named] and the `docs/sound/` exemplars, and verified by the orchestrator against the tree.

- The catalog row placed the `usb2_hardware_lpm` sysfs knob in hub.c. It is defined in `drivers/usb/core/sysfs.c` (`usb2_hardware_lpm_store` at :496, `DEVICE_ATTR_RW` at :525, in the power attribute group at :624, added at :651-660); `hub.c:6196` and `:6309` are calls to the enable and disable helpers inside the reset path. The row is corrected in place.
- The Area F digest gave `PORT_WAKE_BITS` one definition, at `xhci-hub.c:20`. `xhci.c:33` carries an identical second definition, and `xhci_disable_hub_port_wake` (xhci.c) uses that copy. The digest line is corrected in place; the page states both.
...
- SUMMARY carries the domain model in prose plus the state set and its transitions that [purpose.state-sets] requires (one table, one figure at most). Member tables and per-request or per-bit detail sit in DETAILS beside the excerpt they describe ([style.run-on-enumeration]), not in SUMMARY.
- This is the same rule the sound campaign recorded on 2026-08-20 ("counts serve claims; a count that serves no claim is noise; every DETAILS section states what the reader should understand"), which never reached the skill; it is restated here so the xhci writers see it, and surfaced to the user for the skill's rule corpus; it became [purpose.state-sets] later the same day.
...
Surfaced to the user for `guidelines/writing.md` section 6, items 2 and 3; until it is folded in there, this amendment is the campaign's binding statement.
...
3. Create or reuse the machine-local workspace `progress/xhci/` (run log `log.md`, worksheets). It is never committed.
...
16. Drafts: the primitive corpus (see Context CORRECTION) is mineable — "you can get inspiration from them, especially when curating the pages"; "you can reuse the ASCII diagram ... just remember convert them to styles that fit your SKILL.md" (rule 7p and `guidelines/campaign.md` "Deriving from prior drafts and pages" govern).
...
3. Verification cadence: "On demand only" — no scheduled verify campaign; the user triggers `xhci-verify` when wanted. RETIRED 2026-08-31: verification campaigns and the CERTIFIED state were removed from the skill, so this decision no longer has an effect.
...
- PORTSC — `xhci_port_regs.portsc` xhci.h:85; accessors `xhci_portsc_readl`/`writel` xhci.c:51/44. `.PLS`+values `XDEV_U0..RESUME` xhci-port.h:17-30; `.PP`=`PORT_POWER`(bit9) xhci-port.h:33; wake enables WCE/WDE/WOE=`PORT_WKCONN_E/WKDISC_E/WKOC_E`(bits25-27) xhci-port.h:110-114, combined `PORT_WAKE_BITS` xhci-hub.c:20 and, in an identical second definition, xhci.c:33 [CORRECTED 2026-09-02: `xhci_disable_hub_port_wake` uses the xhci.c copy].
...
- Pipeline: writer → orchestrator check per SKILL.md ("Modes") — the page is the writer's end to end (facts and prose, mechanical QA steps run, evidence persisted into the worksheet); the orchestrator re-runs the checks per `guidelines/checking.md` section 7 and stamps WRITTEN → LINTED at batch checkpoints; a page's pipeline ends at LINTED.
...
[errata 2026-09-11: re-entry follows the current skill, SKILL.md "The passes" and the guidelines by phase (SKILL.md "Reading routes"), with the writer brief of `guidelines/campaign.md` [briefs] and the check pass of `guidelines/checking.md` [check-pass]; the pass files, rule files, waivers and rule IDs this section names existed at commit 1bea4ac, before the compaction, and are history, kept here for the record.]
- Per-page procedure: passes 00-03 at write time (`guidelines/passes/` (at commit 1bea4ac, before the compaction)); every writer brief carries this file's boundary statements (its cluster's rules plus rules 11-12), the project bans below, and the write-time cautions below.
...
- semcode is required by the request for research; agents on a machine without a semcode index fall back to Grep/Read against the pinned tree and state so in the worksheet.
...
- pm/usb3-device-pm.md (734 · 22) BACKBONE, weakest of six on completeness; 0/6 location drift BUT 2 confirmed catalog↔DETAILS completeness gaps (a cataloged function whose real body is pure vendor logic — never shown; a helper cited 5× never excerpted) + 1 undisclosed dropped guard in the BOS excerpt; arm×6 CORPUS-WORST, soft cross-page reference evading the .md-link grep; OTHER SOURCES 3/3 fail (0 recoverable). Figures 4/4 reusable (U0/U1/U2/U3 graph with back-edges — strongest in file; two PORTPMSC timeout bitfields; HCSPARAMS3). Mines: U1/U2 timeout pipeline + sysfs catalog → usb3-device-pm rewrite; resolve the two completeness gaps generically (state the policy check's role without naming vendors).
...
6. Depth: xfer-* pages and usb3-device-pm rewrites must deepen (c-block ratios 0.53-0.76 vs the 1.0 floor); usb3-device-pm's two completeness gaps are resolved generically.
7. DOCUMENTATION: never copy a draft's DOCUMENTATION section — fabricated paths (`Documentation/usb/xhci.rst`, `Documentation/driver-api/usb/hcd.rst`) recur across four files; derive entries from the tree's real Documentation/ contents only.
