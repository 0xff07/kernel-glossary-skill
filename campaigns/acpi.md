...
- Pipeline: writer → orchestrator check per SKILL.md ("Modes") and `guidelines/checking.md` section 2/`03-check.md`; the page is the writer's end to end (facts and prose; parity table closed, mechanical QA steps run, evidence persisted into the dossier); the orchestrator adjudicates escalations at batch checkpoints and stamps WRITTEN → LINTED in the run log.
