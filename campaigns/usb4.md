...
5. Pre-rule pages (decision of 2026-09-20): every page of this campaign written before the walkthrough, reading and figure rules, the sixty pages `kg triage` grades fix or rebuild under qa digest 034afa17cce3, is rewritten from its removed revision under the final skill, the fix grade included, because the skill changed too much between their write and now for an in-place fix to be honest work. The fix operation itself stays: a page written under the current rules (the eleven graded current) takes fix passes when a rule moves, and the fix grade keeps its meaning for later campaigns. Each rewrite is a remove-and-add pair deriving from the removed revision (`git show <commit>:<path>`), briefed per references/briefs/writer.md with the removed page as material, never a floor or a ceiling.
...
| topology-scan.md | discovering routers: `tb_scan_switch()` (tb.c:1273) and `tb_scan_port()` (:1289) as one decision tree with every early exit (upstream port, DP OUT with HPD active :1299-1305, non-lane adapter, secondary lane, `tb_wait_for_port` :1318, existing remote, `tb_switch_alloc` failure → `tb_retimer_scan` → `tb_scan_xdomain` on `-EIO`/`-EADDRNOTAVAIL` :1332-1341, `tb_switch_configure` failure, pre-existing XDomain removal :1353-1357, uevent suppression during discovery :1364-1367, `sw->rpm` :1373, `tb_switch_add`), `tb_configure_link()` (:1232: both `remote` pointers, `tb_switch_set_link_width(DUAL)` :1250, the Gen-4 symmetric reconfiguration :1257-1263, `tb_switch_configure_link` :1267), the post-add sequence (retimer scans :1389/1410, CLx :1395, TMU :1400, `tb_switch_configuration_valid` :1407, USB3 tunnels :1418, DP resources :1421, recursion :1422), `tb_scan_xdomain()` (:431), the runtime-PM pairs on the router and the USB4 port (:1277/1282, :1316/1425), the depth limit consequence [errata 2026-09-13, migration of topology-scan.md: the `CLx :1395` anchor is an abbreviation no oracle expands] [errata 2026-09-20, rewrite of topology-scan.md: the runtime-PM pair hints name half of each pair; the router's release is tb.c:1282-1283 (`pm_runtime_mark_last_busy` then `pm_runtime_put_autosuspend`) and the USB4 port's tb.c:1426-1427 under the guard at :1425; the CLx call is tb.c:1397, :1395 being the `if (discovery)` test] | [prompt] |
| router-hotplug.md | the plug journey from interrupt to device: the MSI-X vector firing `ring_msix` (nhi.c:444) and `ring_work` (:268) for ring 0 → `tb_ctl_rx_callback` (ctl.c:445) → `tb_ctl_handle_event` (:402) → `tb_domain_event_cb` (domain.c:338) → `tb_handle_event` (tb.c:2916: `cfg_event_pkg` tb_msgs.h:89, `tb_cfg_ack_plug` ctl.c:842) → `tb_queue_hotplug` (:93, `struct tb_hotplug_event` :77, the domain reference :101 new at v7.2) → `tb_handle_hotplug` (:2421) with the runtime-PM pairs (:2430/2456) and the `hotplug_active` gate → every plug branch (:2503-2517: a lane adapter → `tb_scan_port`; a DP adapter → `tb_dp_resource_available`; the other adapter types) → router enumeration (`tb_switch_alloc`/`configure`/`add`) → the domain additions (USB3 tunnels, DP resources); `tb_handle_notification` (:2885) for the `TB_CFG_ERROR_*` notifications that arrive on the same path; the hotplug-side uevents; the vendor-only port-0 helpers in that branch are named as vendor-only and not documented [errata 2026-09-13, migration of router-hotplug.md: the `configure` and `add` anchors are abbreviations no oracle expands] [errata 2026-09-23, rebuild of router-hotplug.md: domain/domain-start-stop.md catalogs `bool hotplug_active`, and tunnel/dp-bandwidth-allocation.md catalogs `struct tb_hotplug_event` and `tb_handle_notification`; this row and boundary rule 2 assign all three here, so router-hotplug.md catalogs them and the two siblings de-catalog at their rewrites. `usb4_port_hotplug_enable()` and `tb_switch_port_hotplug_enable()` are shown at the arming stage and cataloged by usb4-port-capability.md; the pre-USB4 enable `tb_plug_events_active()` is not named on this page (its body carries per-device rows under ban 1, and `tb_switch_configure()` is router-setup.md's). The title of USB4 1.0 section 6.4.2.7, cited by commit 210e9f56e9e1, is not in the tree, and the page says so; the domain reference the work item holds is commit 138ec65b2c76, first contained in v7.2-rc1] | [prompt] |
| router-unplug.md | the unplug journey: the unplug branch of `tb_handle_hotplug` (tb.c:2461-2476) in order (`tb_retimer_remove_all` retimer.c:592 → `tb_sw_set_unplugged` switch.c:3471 → `tb_free_invalid_tunnels` :1775 → `tb_remove_dp_resources` :138 → `tb_switch_tmu_disable` → `tb_switch_unconfigure_link` switch.c:3227 → `tb_switch_set_link_width(SINGLE)` → `tb_switch_remove` :3430 → clearing both `remote` pointers → `tb_recalc_estimated_bandwidth` :1515 and `tb_tunnel_dp` :2063), the XDomain unplug branch (:2477-2493: `is_unplugged` first, `tb_xdomain_remove`, `__tb_disconnect_xdomain_paths`, `tb_port_unconfigure_xdomain`) and the bus removal outside the lock (`tb_domain_unregister_unplugged_xdomains` domain.c:874 at tb.c:2527), the DP adapter unplug (`tb_dp_resource_unavailable` :2178), the cleanup that runs elsewhere (`tb_free_unplugged_children` :1790, `tb_free_unplugged_xdomains` :3123, `tb_remove_work` :3250 after runtime resume, `tb_complete` :3219), `tb_switch_resume`'s replaced-XDomain branch (switch.c:3611-3624); the adapter-0 branch's `tb_switch_xhci_disconnect` is named as vendor-only and not documented, as the router-hotplug row directs for the same helper pair (orchestrator note 2026-09-05, applied at the check pass) [errata 2026-09-13, migration of router-unplug.md: the `tb.c:2461-2476` span excludes the branch's own opening and the retimer removal at tb.c:2458-2459] [errata 2026-09-23, rebuild of router-unplug.md: router/router-lifecycle.md catalogs `tb_sw_set_unplugged`, which this row assigns here; router-unplug.md catalogs it and router-lifecycle.md de-catalogs at its rewrite. The two `is_unplugged` members are not cataloged here, their structs being tb-switch.md's and xdomain.md's whole; the page draws them in its two state pairs with Lifecycle rows. Stated on the page as fact from the shown lines: `tb_remove_work()` (tb.c:3250) passes `tb->root_switch` to `tb_free_unplugged_xdomains()` after releasing `tb->lock` and without the NULL test that guards the router traversal (:3256-3260), `tb_xdomain_remove()`'s kerneldoc asks for the lock (xdomain.c:2222), and `tb_stop()` cancels the work without waiting (`cancel_delayed_work()`, tb.c:2947) before clearing `root_switch` (:2960), so a callback running when the domain stops can dereference NULL at tb.c:3127; the delayed work runs no bus sweep, so the peers it detaches stay on the bus until a later hotplug event or system resume (commit a8937f35cf39 moved the call outside the lock). Kept off the page as unsettled: a peer marked by the UUID exchange (xdomain.c:1388) can be unregistered by a sweep while `port->xdomain` still names it; `tb_scan_port()` detaches a peer without marking it (tb.c:1353-1356); on a bonded link the surviving adapters keep `bonded` set because `tb_port_lane_bonding_disable()` never runs there] | [prompt] |
...
| usb4-port-capability.md | the USB4 port capability of a lane-0 adapter: `PORT_CS_18`/`PORT_CS_19` bit by bit (tb_regs.h:384-401), `link_is_usb4` (usb4.c:213), `usb4_port_unlock` (:1132), `usb4_port_hotplug_enable` (:1154), `usb4_port_reset` (:1175), `usb4_port_set_configured`/`usb4_port_configure`/`_unconfigure` (:1208/1238/1251), `usb4_set_xdomain_configured`/`usb4_port_configure_xdomain`/`_unconfigure_xdomain` (:1256/1288/1300), `usb4_port_wait_for_bit` (:1305), `usb4_port_clx_supported` (:1574); the router-level users `tb_switch_configure_link`/`tb_switch_unconfigure_link` (switch.c:3198/3227, the downstream-first order that preserves wake-on-connect), `tb_switch_port_hotplug_enable` (:3266), `tb_port_configure_xdomain` (tb.c:416); the pre-USB4 counterpart for hotplug enablement, `tb_plug_events_active` (switch.c:1750) with `struct tb_cap_plug_events` (tb_regs.h:151) and the plug-events VSE registers (:560-578), its vendor branch named as vendor-only; the wake bits are named and owned by pm/wakes.md; router offline and margining are named and owned by sideband pages [errata 2026-09-12, migration of usb4-port-capability.md: the `PORT_CS_18`/`PORT_CS_19` span ends at tb_regs.h:400, the last define; :401 is past the family] [errata 2026-09-23, rebuild of usb4-port-capability.md: siblings on disk catalog symbols this row assigns here, and each de-catalogs at its rewrite: pm/wakes.md (`PORT_CS_18`, `PORT_CS_19`), domain/xdomain.md (`PORT_CS_19_PID`, `usb4_port_configure_xdomain`, `tb_port_configure_xdomain`, `tb_port_unconfigure_xdomain`), sideband/sideband-access.md and adapter/protocol/usb3-adapter.md (`usb4_port_wait_for_bit`), pm/clx.md (`usb4_port_clx_supported`), adapter/link-controller.md (`tb_switch_configure_link`, `tb_switch_unconfigure_link`, `tb_port_configure_xdomain`, `tb_port_unconfigure_xdomain`). `PORT_CS_18_TIP` is bandwidth/asymmetric-links.md's on disk (the asymmetric widths under rule 4), not lane-bonding.md's as the 2026-09-23 brief said; this page names the bit in its map and does not catalog it. `TB_PLUG_EVENTS_USB_DISABLE` (used only at switch.c:1776, inside the vendor branch), the four `TB_PLUG_EVENTS_CS_1_*_DISABLE` bits and `TB_PLUG_EVENTS_PCIE_CMD_BR_MASK`, `_WR`, `_COMMAND_MASK` and `_RD_DATA` (unused at v7.2) are shown and named but not cataloged; the nine `TB_PLUG_EVENTS_PCIE_*` macros `tb_switch_pcie_bridge_write()` uses are cataloged under [registers.section], their one caller being vendor-only. Bit lines on disk, against the brief's ledger hints: CPS usb4.c:1584, CSA :1605, TIP :1695-1696, START_ASYM :1676-1677 and :1688-1689, DPR :1188 and :1202, PC :1222 and :1224, PID :1270 and :1272; the Notification Timeout write is switch.c:2620 under its comment at :2619. Two claims of the removed revision refuted on disk: `tb_init_port()` runs from `tb_switch_add()` (switch.c:3339), not at allocation; both callers of `tb_switch_reset()` pass the host router (tb.c:3048, :3155), so a device-router reset is unreachable at v7.2. Rulings: the vendor branch of `tb_plug_events_active()` (switch.c:1764-1777) is exempt from [excerpts.walkthrough] under ban 1; a Lifecycle table whose object is a register bit (`PORT_CS_19_PC`) is exempt from the struct-based [evidence.lifecycle] resolution, its rows verified by hand at the check pass] | [prompt] |
...
| retimer-enumeration.md | retimers as devices: `struct tb_retimer` (tb.h:339, kerneldoc :326), `tb_retimer_scan()` (retimer.c:510) step by step (`usb4_port_enumerate_retimers` usb4.c:1556, the status pre-read `tb_retimer_nvm_authenticate_status` retimer.c:197, inbound SBTX guarded by the offline state `tb_retimer_set_inbound_sbtx`/`_unset` :214/231, `usb4_port_retimer_is_last`/`_is_cable` usb4.c:1913/1939, `TB_MAX_RETIMER_INDEX` retimer.c:18/20), `tb_retimer_add()` (:389), `tb_retimer_remove()` (:465), `tb_retimer_release()` (:376), `tb_retimer_type` (:383), `retimer_match`/`tb_port_find_retimer` (:478/486), `remove_retimer`/`tb_retimer_remove_all` (:576/592, reverse order), the attributes `device`/`vendor`/`nvm_version`/`nvm_authenticate` and `retimer_is_visible` (:169-374) with ABI (:339-370), runtime PM (:454-459), debugfs init/remove seams (:461/468), the callers (tb.c:1332/1389/1410, usb4_port.c:91/240) and the removal seams; the absence of a resume path stated [errata 2026-09-13, migration of retimer-enumeration.md: the `tb_retimer_set_inbound_sbtx`/`_unset` shorthand resolves in the oracle to the family stem `tb_retimer_unset`, which is not a symbol; the symbol is `tb_retimer_unset_inbound_sbtx` at retimer.c:231. Nothing is missing from the page, and the row's spelling is what produces the finding] [errata 2026-09-23, rebuild of retimer-enumeration.md: sideband/sideband-access.md still catalogs `usb4_port_enumerate_retimers` and `TB_MAX_RETIMER_INDEX`, this row's under rule 6, and de-catalogs at its rewrite; sideband-access.md and adapter/usb4-port-device.md both catalog `usb4_port_device_is_offline` and `usb4_port_offline`, which settles at sideband-access.md's rewrite under rule 6 (router offline is its); `nvm_version`, `nvm_authenticate` and `tb_retimer_nvm_authenticate_status` are retimer-nvm.md's by rule 6 and review item 10, the row's listing of the two attributes notwithstanding. Two claims of the removed revision refuted on disk: e34f1717ef06 added `retimer_is_visible()` whole with the write at retimer.c:109, not `nvm_version` alone; the -EOPNOTSUPP of a retimer's NVM add is not reachable through a missing `read_version` at v7.2, and the page states what the code guarantees. Two behaviors the page states as fact, for the reader: the attribute visibility is evaluated at `device_register()` (retimer.c:437) before `tb_retimer_nvm_add()` (:444) can set `no_nvm_upgrade` (:109), so a retimer with an unknown firmware format keeps its NVM files visible; `tb_remove_work()` calls `tb_free_unplugged_xdomains()` after releasing `tb->lock` (tb.c:3258-3260)] | [prompt] |
...
| rings.md | the descriptor rings: `struct tb_ring` field by field (include/linux/thunderbolt.h:563, kerneldoc :533), `struct ring_desc` (nhi_regs.h:34) and what the hardware writes back, `enum ring_desc_flags` (thunderbolt.h:608), `struct ring_frame` (:627), the ring registers `REG_TX_RING_BASE`/`REG_RX_RING_BASE`/`REG_TX_OPTIONS_BASE`/`REG_RX_OPTIONS_BASE` (nhi_regs.h:52-82) with `ring_desc_base`/`ring_options_base` (nhi.c:171/179) and the `ring_iowrite*` helpers (:187-214), `ring_full`/`ring_empty`/`tb_ring_empty` (:219/224/712), `ring_write_descriptors` (:234), `ring_work` (:268) and the callback context, `__tb_ring_enqueue` (:320) with `tb_ring_rx`/`tb_ring_tx` (thunderbolt.h:682/703), the lifecycle `tb_ring_alloc` (:530) → `nhi_alloc_hop` (:458) → `tb_ring_alloc_tx`/`_rx` (:603/626) → `tb_ring_start` (:642) → `tb_ring_stop` (:751) → `tb_ring_free` (:796), `tb_ring_flush` (:728) with `ring->wait` (new at v7.2), `tb_ring_size`/`tb_ring_frame_size` (thunderbolt.h:648/641), `tb_ring_dma_device` (:724), the lock model (`nhi->lock` then `ring->lock`, the callback run unlocked), `hop_count` and `RING_NOTIFY_REG_COUNT`/`RING_INTERRUPT_REG_COUNT` (nhi_regs.h:91/100) [errata 2026-09-13, migration of rings.md: the allocator pair is written `tb_ring_alloc_tx`/`_rx`, and no oracle expands `_rx`, the same shorthand the domain and router clusters already record. The symbol is cataloged, excerpted and paired on the page] [errata 2026-09-23, rebuild of rings.md: host-if/tb-nhi.md and adapter/hopid-allocation.md still catalog `nhi_alloc_hop`, hopid-allocation.md also `RING_FIRST_USABLE_HOPID` and `RING_E2E_RESERVED_HOPID`, and host-if/ring-modes.md still catalogs `REG_TX_OPTIONS_BASE`, `REG_RX_OPTIONS_BASE`, `tb_ring_alloc_tx`, `tb_ring_alloc_rx`, `tb_ring_start`, `tb_ring_frame_size` and `tb_ring_dma_device`, all assigned here by rule 7 and this row; each de-catalogs at its rewrite. Boundary settled at the check pass: `RING_NOTIFY_REG_COUNT` and `RING_INTERRUPT_REG_COUNT`, which this row names, are msi-msix.md's catalog entries under rule 7 (interrupt masking) and rings.md locates them; `nhi_pci_ring_request_msix`/`nhi_pci_ring_release_msix` are cataloged by msi-msix.md and only cited here] | [prompt] |
