...
Campaign short name: `usb4`. Campaign file: `campaigns/usb4.md`; workspace directory: `progress/usb4/` (run log, dossiers, agent intermediates; nothing outside it). Name collision check on 2026-09-04: no `campaigns/usb4.md` and no `progress/usb4/` existed.
...
3. Create or reuse the machine-local workspace `progress/usb4/` (run log `log.md`, dossiers). It is never committed.
...
Amendment 2026-09-04 (user, later the same day; supersedes decision 4 for Corpus 2 only): the ASCII figures of the kernel-glossary-devel corpus (`kernel-glossary-devel/docs/usb4/` at TREE_ROOT, v6.19) MAY be reused, but only the figures that draw registers, configuration spaces and other bit-field layouts. The user's words: "The ASCII diagram in the old drafts may be reused, but only for the registers, config spaces, and other bitfield-oriented diagrams. Don't reuse diagrams exibiting code flow in the old drafts because they may change in newer kernel here." How this binds a writer:
   ...
   - [facts.derived-pages] governs the derivation: the dossier records the source figure as an inventory item with its disposition (kept and redrawn, or cut with the reason), and the page's own parity audit is unaffected by what the source covered.
   - Corpus 2 is untracked at TREE_ROOT and may be absent on another machine; where it is absent, the writer draws the figure from the tree and the dossier says so. Corpus 1 (the June corpus under `docs/usb4/`) stays at decision 4: no pointers of any kind.
...
- Pipeline: writer → orchestrator check per SKILL.md ("Modes"). The page is the writer's end to end (facts and prose, the mechanical QA steps run, the evidence persisted into the dossier); the orchestrator re-runs the checks per `guidelines/checking.md` [check-pass] and stamps WRITTEN → LINTED at the slice checkpoint, recording `LINTED <date> page sha256: <digest>` in the dossier's LINT; a page's pipeline ends at LINTED.
...
- Write-time cautions (line numbers in this file are hints; re-verify on disk). The per-area drift ledgers (Inventory findings, item 11 of each digest) are binding, and every writer brief carries the items below:
  - (1) Both draft corpora predate v7.2: every symbol taken from either is re-found by name on disk, and a symbol that does not survive `git grep` at v7.2 is reported in the dossier, never written around.
