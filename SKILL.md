...
Producing one page is four passes over three artifacts, and a campaign adds a plan pass in front (`guidelines/campaign.md`). No pass creates an artifact beyond the three: the page, the spec and the workspace with its log, worksheets and first-pass records.
...
3. The run workspace, `progress/<campaign>/`: the run log (`log.md`), the worksheets, one per page at `progress/<campaign>/<dir>/<group>/<slug>.worksheet.md`, mirroring the page's path `docs/<dir>/<group>/<slug>.md` because two groups of one subsystem may share a slug, and the first-pass records beside them; machine-local and never committed, the crash-recovery point for in-flight work. Nothing in it travels, and nothing in it is trusted: every entry is re-verified on disk before it is used, every row a check reads is matched to the current page or the tree, and the engine checks no identity on the file.
