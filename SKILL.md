...
The parity table, the lint report, and the verify report are SECTIONS OF THE DOSSIER, not files beside it — a pass records its outcome in the dossier and reports it in its final message, which is what the orchestrator actually reads. Helper scripts and working scratch belong in the agent's own scratchpad directory, never in `progress/`. `progress/` accumulates, and finished, suspended, and abandoned workspaces stay on disk until the user deletes them.
