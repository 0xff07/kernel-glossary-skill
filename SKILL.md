...
| 02 write | `guidelines/passes/02-write.md` | dossier → substance-verified page at `docs/<dir>/<slug>.md`, plus the closed parity table at `progress/<campaign>/<slug>.parity.md` |
| 03 style lint | `guidelines/passes/03-lint.md` | page → adjudicated stylistic finding list at `progress/<campaign>/<slug>.lint.md` (page unchanged; a patch agent applies the reviewed fixes in a campaign, a solo agent applies its own) |
| 04 verify | `guidelines/passes/04-verify.md` | page after style fixes → Gate A/B outcomes recorded in `progress/<campaign>/<slug>.verify.md`; page final |
...
Multi-page campaign (a documentation set of tens of pages): plan first per `guidelines/campaign/planning.md` (a campaign starts with a unique short name and workspace under `progress/` per `guidelines/campaign/progress-layout.md`, then a plan the user approves at `progress/<campaign>.md`), then produce pages in batches through the writer → style lint → patch → verify pipeline per `guidelines/campaign/pipeline.md`, dispatching sub-agents with the role cards and brief templates under `guidelines/agents/`. Ownership splits by kind, not by gate: the writer owns all disk-settleable substance — it composes under every rule, closes the parity table at zero empty rows, and runs the mechanical exit suite (excerpts, anchors, coverage, second-basis counts) before reporting done — but never sweeps its own prose; the style-lint agent finds and adjudicates prose/form defects only (find-only, no facts, no parity); a patch agent applies the reviewed stylistic fixes; the orchestrator runs final verify as the pipeline's only independent fact-check and never delegates sign-off. Substance defects found downstream always return to the writer, never to the patcher. In a campaign whose page catalog the user has already approved, save each finished page without a per-page ask and checkpoint per the pipeline; git commits still require an explicit user go.
