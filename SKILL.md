...
- After the writing session: a writer transcript is never resumed across sessions, so every further finding is surfaced to the user and, on their go, routed to a fixer in fix-list mode (appliable classes) or to a fresh writer started from the dossier plus the campaign spec. That pair replaces the dead transcript, which is why the writer persists its evidence before reporting done.
