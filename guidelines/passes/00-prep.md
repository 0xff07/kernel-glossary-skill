...
- pages rebuilt from earlier drafts: the archetype match above for form; PLOT-04 governs the derivation itself, and `guidelines/reference/draft-contrast.md` remains the record of what a rebuild owes over the draft it starts from

If no archetype matches, pick the structurally closest `docs/sound/` page anyway. Do not calibrate against pages under any other `docs/` directory: they were written under earlier or since-withdrawn rules. The frozen copies under `guidelines/reference/samples/` were retired as the writer's exemplar on 2026-09-02, for the reasons recorded in `guidelines/reference/measured-criteria.md`, and are not read before writing a page.
