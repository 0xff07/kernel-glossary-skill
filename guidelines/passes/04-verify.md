...
4. Findings. Solo: fix and re-check now — the solo agent is the writer, so substance is in its hands. Verify campaign: record every finding in the report, find-only; the orchestrator adjudicates and routes (appliable fixes to a fixer in fix-list mode per `guidelines/passes/03-lint.md`; structural defects become rewrite decisions surfaced to the user), then the touched units are re-checked.
...
5. Fix routing: findings the orchestrator confirms as appliable go to a fixer in fix-list mode (`guidelines/passes/03-lint.md`); structural findings (a wrong model, a scope breach, a parity hole needing research) become rewrite decisions surfaced to the user — executed, when approved, by a fresh writer started from the dossier and the plan file, never by resuming a writer transcript across sessions.
...
- The campaign is complete when every corpus page is CERTIFIED or explicitly recorded as failed or waived by the user. Residual false-positive classes become LESSON entries and are folded into the 7r registry.
