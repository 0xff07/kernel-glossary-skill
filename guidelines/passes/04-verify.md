...
Outputs: the dossier's VERIFY section, recording the Gate A and Gate B outcomes with their evidence, plus the CERTIFIED entry the orchestrator stamps into the campaign file's Status. No report file.
...
3. For a derived page, confirm every 7p cut was reported (and, in a campaign, recorded in the campaign file) before the page can be certified (`guidelines/rules/rules.md` (7p)).
4. Findings. Solo: fix and re-check now — the solo agent is the writer, so the facts are in its hands. Verify campaign: record every finding in the dossier's VERIFY section, find-only; the orchestrator adjudicates and routes (settled style and byte-proved drift to a fixer in fix-list mode per `guidelines/passes/03-lint-fixlist.md`; every factual finding into the campaign file's Status — a verify campaign never edits facts), then any fixer-touched units are re-checked.
5. Record the outcome of Gate A and Gate B in the dossier's VERIFY section. Zero unadjudicated findings certifies the page (state CERTIFIED in the campaign file's Status); any unconfirmable item leaves it uncertified — solo, the page is not written.
6. Residual false-positive classes are recorded as LESSON entries in the campaign file's Status and surfaced to the user, who alone folds a ruling into the 7r registry.

## The verify campaign

Verification at campaign scale is a CAMPAIGN LIKE ANY OTHER — same campaign file, same structure, same batching, same resumability (`guidelines/passes/plan.md`). The only difference is what its catalog lists: pages to CERTIFY, rather than pages to write. There is no separate plan format, no certification census, and no delta catalog; the campaign file's page table IS the census, and its confirmed factual findings ARE the catalog of the follow-up write campaign that repairs them.
...
Workspace and the parent carve-out: a verify campaign is its own run under `progress/`, named `<parent>-verify`, with the same two entries as any campaign — the campaign file `progress/<parent>-verify.md` and the dossier directory `progress/<parent>-verify/`. Its Context declares the parent (or "standalone"); the user's request to verify licenses reading the parent's campaign file and dossiers as hints — the one exception to the read-nothing isolation rule, and never as evidence. The parent's entries are not modified; the one cross-write is the orchestrator mirroring CERTIFIED stamps into the parent's Status.

Planning is orchestrator-owned — the same rule as catalog design. The verify campaign file carries the standard sections, with these specializations:

1. Context: the corpus (page list with absolute paths), the parent campaign or "standalone", and the documented tree with version tag and commit pin — re-derived and confirmed against the corpus's own version claims before anything runs, because a verifier on the wrong tree produces confident nonsense.
...
4. Status: the living log — batches dispatched, findings adjudicated, fixes applied, pages CERTIFIED, and every confirmed factual finding with its evidence and (where derivable) its exact fix specification. A later session resumes from here, and this is what seeds the repair campaign.
...
- The only in-place edits are registry-settled style fixes and byte-proved drift corrections, applied by a fixer in fix-list mode (`guidelines/passes/03-lint-fixlist.md`) and re-checked before any page's state changes. Every FACTUAL finding — a parity hole, a false count, a drifted excerpt, a wrong model, a scope breach — is recorded in Status and the page stays uncertified; facts are repaired only by a follow-up write campaign whose catalog is those findings, never in place and never by resuming a writer transcript.
- A page with zero unadjudicated findings is stamped CERTIFIED in Status (mirrored into the parent's Status when there is one).
...
PROJECT-SPECIFIC BANS from the parent campaign file: <list, or "none">.
...
CHECKS, from the verify campaign file at <path>:
1. Seam consistency: for each boundary statement and seam symbol listed
   in the campaign file, confirm every sibling page states the seam identically
...
4. <Any campaign-specific cross-page claims from the campaign file.>
