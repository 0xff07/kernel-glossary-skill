...
4. Findings. Solo: fix and re-check now — the solo agent is the writer, so the facts are in its hands. Verify campaign: record every finding in the dossier's VERIFY section, find-only; the orchestrator adjudicates and routes (settled style and byte-proved drift to a fixer in fix-list mode per `guidelines/passes/03-lint.md`; every factual finding into the campaign file's Status — a verify campaign never edits facts), then any fixer-touched units are re-checked.
