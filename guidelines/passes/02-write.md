...
10. Persist the evidence: append the suite's outcomes to the dossier's EVIDENCE section (`guidelines/passes/dossier.md`) — every count and universal claim with its two derivation bases and reconciled result, the excerpt-unit and anchor-confirmation tallies, the span-closure result, the excerpt-explanation list (per definition block, members shown, members named, and the group phrase covering each unnamed member), the leading-paragraph list (PAGE-08: each leading paragraph's first sentence with its verdict, and SUMMARY's table and figure counts), and the sweep candidate counts per class with their dispositions (was the Gate A record) — so the check pass re-derives from recorded bases instead of reconstructing them.
