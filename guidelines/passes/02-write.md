...
9. Persist the evidence: append the suite's outcomes to the dossier's EVIDENCE section (`guidelines/passes/dossier.md`) — every count and universal claim with its two derivation bases and reconciled result, the excerpt-unit and anchor-confirmation tallies, the span-closure result, and the sweep candidate counts per class with their dispositions (was the Gate A record) — so the check pass re-derives from recorded bases instead of reconstructing them.
