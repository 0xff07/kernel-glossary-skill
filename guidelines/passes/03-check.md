...
Purpose: confirm, independently and mechanically, that the writer's own checks were actually run and actually held. This pass does NOT redo the writer's work and does NOT read the page for style — it re-runs the writer's procedures against ground truth and compares the answers. Its one reach into prose content, the excerpt-explanation check (step 7), is bounded the same way: a generator counts the members each paragraph names, and the orchestrator reads only the residue it prints.
...
7. **Excerpt explanation (PAGE-07).** Re-run PAGE-07's generator over the raw file and compare its rows with the dossier's excerpt-explanation list. Every zero-named block is a finding; every unnamed member must match a group phrase the writer recorded, and you re-read a sample of those phrases against the excerpt's own comments (a phrase that names no grouping the excerpt shows is a bare member dressed up as covered). Then read the paragraph beside two or three of the fully named blocks for the shape-only paragraph the generator cannot express: counts, positions, lifetime partitions standing in for what the members hold. This is the one check in this pass that reaches prose content, and it is here because the generator makes it mechanical: it counts names, and the orchestrator judges only the residue. Measured 2026-09-02: the two batches that shipped before this step existed ran 51% of their definition blocks at zero named, against 13% to 25% for every corpus written before the rule changes that produced them.
