...
- `progress/` is gitignored, with the committable exception (top-level plan files) defined in SKILL.md ("The progress/ workspace"). A run's artifacts are disposable once its campaign closes (keep them until then; enhancement passes, gap-fill writers, and verify campaigns reuse them). Another run's artifacts are off limits unless the user explicitly directs resume or reuse (verify campaigns read their declared parent under that rule).
