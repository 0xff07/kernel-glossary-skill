...
2. Status: a living, dated checklist; a future session resumes from this section plus the pages on disk. Campaign-level events (phase completions, batch results, suspensions, user amendments, corrections, lessons) are appended here at the moment they happen; the per-page ticker accumulates in the run's status journal (`progress/<campaign>/<campaign>.journal.md`, local scratch) and is folded into this section at batch checkpoints, so the committable plan file stays clean between checkpoints. Per-page state is tracked as WRITTEN → LINTED → CERTIFIED: the first two are stamped by the write campaign's checkpoints, CERTIFIED only by a verify campaign (`guidelines/passes/04-verify.md`), which mirrors its stamp into this section when this campaign is its parent.
