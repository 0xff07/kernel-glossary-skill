...
5. Write the boundary rules. Self-contained pages overlap by design, so for every cluster of sibling rows write one boundary statement that fixes each page's mission. **A boundary statement is a FLOOR, not a fence.** It says what a page must cover exhaustively; it never says what a page may not touch. Ownership means "must cover exhaustively" — the definition excerpt, the lifecycle, the locking, the enumerated call sites — and it does not mean "may cover exclusively". Any other page reaches the same symbol to whatever depth its own narrative needs, and stops where the code stops being about its subject. Two pages citing one function is correct when one owns it and the other reaches it; a reader stranded at a seam is not, and there are no cross-page links to rescue them. The useful form names the seam symbol and the depths on each side: "page A owns the syscall surface exhaustively and reaches into the X machinery only as far as the syscall drives it; page B owns X's object pipeline exhaustively; helper Y at file:line is where A's treatment thins and B's becomes exhaustive". These statements go into the campaign spec and later verbatim into each writer brief. The discipline that keeps overlap honest is the ground-truth rule, not the boundary: a page never takes a fact, a line number, or an excerpt from another page, so two pages covering one symbol each re-derive it from the tree and cannot drift from the kernel.
...
8. Checkpoint with the user before generating anything. Ask only the genuine scope questions, each with two to four concrete options (include a supporting construct group or not; cover a full syscall surface or a subset). Present the final catalog and directory layout, and get an explicit go. The go approves the CATALOG; it does not start generation — execution happens only through user-invoked slices (SKILL.md, "The three artifacts and the two states"). Record the questions, the answers, and every later amendment (priority reorders, pipeline changes, new bans) in the spec's dated amendments at the moment it arrives; amendments supersede the original order silently otherwise, and a superseded ordering stays in the spec marked as reference.
...
Per-page pipeline states (WRITTEN → LINTED) are recorded only in run logs, never in the spec (SKILL.md, "The three artifacts and the two states").
...
A repair campaign is an ordinary write campaign whose catalog comes ready-made: the confirmed findings the user or the orchestrator has accumulated against pages already on disk. There is no separate plan format and no separate artifact — planning collapses to lifting those findings into a catalog (a spec at `campaigns/<name>.md` like any other), one row per page to repair, carrying each finding with its evidence and, where one was derived, its exact fix specification. The tree pin, the boundary statements, and the project-specific bans come from the parent campaign spec.
