...
Outputs: the page with lane-1 fixes applied, and the dossier's LINT section written (`guidelines/passes/dossier.md`): every candidate found, each FIXED (with the exact before/after), ESCALATED (with a proposed fix), or EXEMPT (with the 7r ruling applied). Page state after this pass: LINTED.
...
Lane 2 — escalate find-only. Everything else: candidates without a settled ruling, 7d superlatives whose in-context judgment is not obvious, heading-truth questions, anything touching the page's mission or boundary, and every suspected factual defect (wrong count, dubious claim, parity hole, excerpt content mismatch) — the last class goes in the LINT section's FACT NOTES subsection for the verify stage; do not act on it.
...
5. Write the dossier's LINT section: FIXED items grouped by class with counts and the exact before/after per fix; ESCALATED items each with location, exact current text, and a fix stated precisely enough to apply without judgment; EXEMPT candidates with the ruling applied (state the 7r ruling count as proof of the registry read); FACT NOTES last. That section is the whole output; do not create a report file. Helper scripts and scratch go in your own scratchpad directory.
