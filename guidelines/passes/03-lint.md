...
Outputs: the page fixed in place, plus a lint report at `progress/<campaign>/<page-slug>.lint.md` (the run's artifact directory, `guidelines/campaign/progress-layout.md`) covering findings fixed by class with counts, 7o corrections with the search evidence, parity gaps, candidates adjudicated as exempt with reasoning, and residual items.
...
6. Beyond the 7o corrections, do not change facts, scope, or structure; this is a compliance pass. Any helper script or intermediate file you persist goes in the run's artifact directory `progress/<campaign>/` under a unique `<page-slug>.`-prefixed name (shared names have collided between concurrent lint agents).
