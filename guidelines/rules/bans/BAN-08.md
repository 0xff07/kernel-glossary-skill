...
8. AN EXCERPT IS THE TABLE. The members a fenced definition excerpt shows are explained in the paragraph beside it, member by member, and that walk is outside this rule however many members it covers: the reader has the excerpt in view, which is the comparison surface a table would have supplied. What this rule forbids there is the same thing it forbids everywhere, one sentence stringing the names together; the walk is a sentence or two per member. Rule 3's "what its members share" is what the prose keeps ABOVE A TABLE, and it was misread the week this rule landed: two batches of pages moved every field's purpose into a table in another section and left the paragraph beside the excerpt saying how many fields the construction pass writes. When members also warrant a table (a role, a writer, and a location per member), the table sits beside the excerpt, not in another section, and the paragraph beside the excerpt still explains what it shows. Added 2026-09-02.
...
1. Read every enumerating sentence in DETAILS, SUMMARY, and the lead; no candidate pattern expresses this shape, so it is a read-through. A member-by-member walk in the paragraph beside the definition excerpt that shows those members is not a candidate (rule 8); a single sentence stringing the members' names together is, wherever it sits. The generator is a grep for sentences carrying three or more commas plus "and"; rank the hits by how many DISTINCT file:line locations each carries, because a sentence with four locations is almost always a real hit and a sentence with one is almost never. Split sentences on semicolons as well as full stops, or an eleven-member enumeration written with semicolons is invisible to the generator.
