...
- Anthropomorphic placement verbs: sweep the full lemma sets `live/lives/lived/living`, `sit/sits/sat/sitting`, `hang/hangs/hung/hanging`, `want/wants/wanted/wanting` (base forms included, not just three inflections; this list is the authoritative lemma set the adjudications registry refers to) and judge each: banned for code, data, and physical devices alike in authored prose per the settled adjudications registry (`../7r-adjudications.md`); a userspace process as a real actor is exempt ("the reader wants the buffer"), the adjective "live" ("the live counter") is not the verb, and verbatim quotes keep their verbs. Confirm "walk" appears only for traversing a data structure.
