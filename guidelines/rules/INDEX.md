...
The sample pages under `guidelines/samples/` embody every rule in this directory. The closest-matching sample read in the prep pass (`guidelines/passes/00-prep.md`) is the worked example for a new page; match its structure, diagram style, code-citation density, and depth. The examples in the rule files use ACPI and mm symbols; they illustrate the rule mechanic, which applies unchanged to every subsystem. All generated content must follow these rules.

Rule IDs are stable identifiers: briefs, lint and verify reports, plan files, and the gates cite rules by ID, and the IDs never renumber. 7g-7i are the diagram rules and live under `guidelines/diagrams/` beside their reference figures.
...
| 7g | `guidelines/diagrams/7g-principles.md` | General ASCII diagram principles (mandatory) |
| 7h | `guidelines/diagrams/7h-register-bitfield.md` | Register and bitfield figures (mandatory) |
| 7i | `guidelines/diagrams/7i-patterns.md` | Other ASCII diagram patterns |
...
The gates that enforce these rules are `guidelines/gates/gate-a.md` (mechanical grep gate), `guidelines/gates/gate-b.md` (nine-item review sign-off), and `guidelines/gates/mechanical-checks.md` (the by-hand check procedures both gates use).
