...
5. Material citing old rule IDs resolves through the Legacy ID map in Appendix B.
...
The recipe rows map to their rules:
...
3. Per figure, in precedence order:
   ...
   3. DIAG-03's criteria for bit layouts, on top of DIAG-01's general rules (DIAG-01's under-80-columns rule takes its one exception from this style's single-row L-connector register); queue its bit ranges, constants, and macros for Step 10's claim audit;
