...
2. A diagram earns its place when it shows physical layout, parallel structure across multiple lanes, a non-linear graph, an address space, a bit field, a ring/queue with head and tail pointers, or two views of the same data side by side, or a quantity compared by length or height on one axis.
...
17. Use Unicode box-drawing characters for sides, corners, junctions and connectors: the light set `┌ ┐ └ ┘ │ ─ ├ ┤ ┬ ┴ ┼`, the double-line set `═ ║` for emphasis, and the dashed vertical `╎` for a guide or a barrier; `▼ ▲ ◀ ▶` for arrows; the block elements `█ ▒ ░` for a filled bar; and `·` for a placeholder cell. Nothing else draws a line or a fill. (Amended 2026-09-03: the earlier list was read as exhaustive by every checker an agent wrote, while `═` and `░` were already in the corpus and its exemplars.)
...
1. For each figure, name the spatial, temporal, transformational, or quantitative relationship it conveys (a physical layout, parallel lanes, a non-linear graph, an address space, a bit field, a ring with head and tail, two views of one data structure, a quantity compared by length or height on one axis). A figure for a linear call chain, a two-state toggle, or anything one declarative sentence conveys is deleted and the sentence written instead.
