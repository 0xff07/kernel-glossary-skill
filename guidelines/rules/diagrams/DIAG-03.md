...
- Scope: apply this rule to every figure whose primary subject is a bit layout (a register, TRB, context, descriptor, packet header); a bit strip that is one element of a larger structural figure follows the host figure's style instead.
- Run the register-versus-structure test and record the answer (one value, or several separate words at successive DWORD offsets). A structure is drawn DWORD-grid, never L-connector; a register is DWORD-grid when its field names fit the cells and L-connector when it is mostly single-bit fields; an L-connector register is a single row of all its bits, or just the low DWORD with a continuation note when the upper bits are one uniform field.
