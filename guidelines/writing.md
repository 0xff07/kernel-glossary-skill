...
Sweep every pattern below over a prose view of the page (fences dropped, links reduced to their text, code spans, quoted text and file:line citations masked), case-insensitively and never anchored to line start, because a paragraph is one line and an anchored pattern sees only its first clause (a heading sweep anchors, since a heading is one clause), and again after every edit. The table states each ban; its `tools/qa/checks/<id>.py` module implements the sweep, with dots and hyphens in the ID replaced by underscores. A pattern generates candidates and the exempt column decides; an exempt construct is never reworded to silence a pattern, and a new exemption lands only through the user. An exemption a regex can name is implemented and tested in the check; one that needs reading is recorded in the dossier's LINT as `EXEMPT <rule-id> "<a fragment of the flagged text>": <the ruling applied>`, which `kg check` reads (dossier.md [lint]).

| ban | what is swept | fix | exempt | requirement |
|---|---|---|---|---|
| em dash | the em dash character | parentheses, or two sentences | a verbatim quotation; figure text is swept | style.em-dash, qa |
| boldface | `**` on the raw file | plain text | `/**` kerneldoc openers inside fences | style.boldface, qa |
| negative construction ("X, not Y", "X rather than Y") | a denial joined by a comma or "and", "rather than" and "instead of", then read | state what the thing is | a quoted comment; a comparison whose denied alternative is a real design the reader could otherwise assume | style.negative-construction, qa |
| placement verbs | the lemmas live, sit, hang, want | "is defined in", "is held in", "occupies", "is attached behind"; a field "transitions through" its values | the adjective ("the live ring"); a real actor; a verbatim quote; hardware earns no exemption | style.placement-verbs, qa |
| "walk" for a scalar | the verb "walk", then read | "advances through" | traversing a data structure | style.walk, qa |
| "vtable" | the word | "function pointer struct" or the type name | nothing | style.vtable, qa |
| counting headings | a heading opening on a cardinal number, word or digit; every DETAILS H3 and H4 read | a heading naming what the section is about, with the count moved into its opening paragraph | nothing | style.counting-headings, qa |
| question or bare-noun headings | a heading opening on Why, How, Where or What, or ending in a question mark, and a DETAILS H3 with no word that reads as a verb, listed for reading; every DETAILS H3 and H4 read | a declarative what-does-what heading | the H3 catalog labels of COVERAGE | style.headings, qa |
| label-colon prose ("X: Y", "is the key:", "says: quote") | a short label ending in a colon before a word, a digit or a span, outside table cells | one plain declarative sentence; a quote as "According to the comment <quote>, ..." | the paragraph-final colon that introduces the fenced excerpt on the next non-blank line; colons in H3/H4 labels, link titles, code, URLs, ratios, catalog bullets, table cells and double-quoted text | style.label-colon, qa |
| intro sentence plus list | every list under DETAILS or SUMMARY, which fails; the lead's is [lead-summary.lead]'s | fold the items into one flowing paragraph or a table | the catalog lists of COVERAGE, DOCUMENTATION and OTHER SOURCES; the entry bullets of section six; tables | style.lists, qa |
| hollow superlatives and importance frames | the words listed below, the "is what" and "matters" frames and "the reasoning"; every adjective read with the deletion test | name the mechanic in the same clause and drop the ranking | "fast path" and "slow path" where the kernel names them; verbatim quotes | style.superlatives, qa |
| banned words | contract, tally in every form, canonical, and the noun arm for a branch or case | the rule, count or helper the word stood in for; "branch", "case", "side" | Arm, ARM64, arm64; the verb ("arms a timer"); verbatim quotes | style.banned-words, qa |
| hedges | usually, typically, generally, often, normally, commonly, mostly, in practice, tends to, on a hot CPU, simply, essentially, basically, arguably | name the exact condition the code tests | hyphenated compounds; a hedge inside a quote; a measured statistic that cites its counter | style.hedges, qa |
| verification narration | grep, semcode, the caller index, a search or second basis, re-derived, verified negative, verified absent, and a tree-wide search, scan or sweep | delete the method clause and keep the result with its scope condition | verbatim quotations; a page about search tooling; a scope condition is never a hit | style.verification-narration, qa |
| run-on enumeration | a sentence carrying four or more members of one set (three commas plus " and ", ranked by distinct file:line locations, read) | a Markdown table, one row per member, every construct linked in every row | the steps of one operation; the member-by-member walk beside the excerpt that shows those members | style.run-on-enumeration, qa |
...
The bans reach body prose and headings, and figure text for the em dash, the negative construction and the placement verbs; never a source excerpt or a verbatim quotation.
