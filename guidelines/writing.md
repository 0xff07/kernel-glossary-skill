...
6. [excerpts.outline, qa] A function walked in two or more pieces is outlined before its first piece: a table with the columns piece, lines and stage, one row per piece in order, `① tb.c:319-332 asks for enhanced uni-directional`, and each piece's introducing sentence carries its mark, the alphabets alternating between consecutive series (figures.md [drawing.marks]), so the reader holds the shape of the function before reading it and always knows which piece this is. The check matches the outline's rows to the pieces shown and the marks to the sentences above the fences.
