...
2. [drawing.never-chains] Never draw a linear call chain, a top-down call chain, a two-state toggle, or a flow that reads as a paragraph; "A calls B which calls C" is prose, and a single arrow chain in a box is not a diagram.
...
## Banned shapes and the strip test [shapes]

1. [shapes.banned] Four shapes fail however true their content, and this outranks every pattern below: a plain table, a `┌─┬─┐` grid of records by attributes, which belongs in a Markdown table (a grid is a figure only when its cells partition something); a plain function-flow graph, nodes that are all function names joined by edges that all mean "calls" (edges that mean feeds, points at, transitions to, is claimed by, gates or is written by are structure and are welcome; a swimlane that would survive deleting all but one lane is this shape wearing lanes); a plain listing of struct members, a box nothing exits; and plain text in a fence.
2. [shapes.strip-test] Strip every label and read what is left. A figure survives when the skeleton still asserts something (this contains that, this becomes that, these ends meet here, this partitions into those, time runs this way across these actors) through a spatial property: position on an axis, a length, containment, alignment, the direction of a flow. A box is a container, not a shape; a column of boxes whose meaning is in their labels fails, and so does an axis whose positions mean nothing.
...
1. [geometry.unicode, qa] Lines, corners and junctions come from the Unicode box-drawing block, in the weight the figure needs: the light set `┌ ┐ └ ┘ │ ─ ├ ┤ ┬ ┴ ┼` by default, the double-line `═ ║ ╔ ╗ ╚ ╝ ╤ ╧` for emphasis or a construct of another kind, the dashed `╎ ┆ ╏` for a guide, a barrier or a second class of path, `╳` for a crossing; fills come from the block elements `█ ▒ ░`, arrowheads and markers from the arrows and geometric shapes (`▼ ▲ ◀ ▶`, `◉`), and `·` marks a placeholder cell. Within one figure one glyph means one thing. ASCII `\`, `/` and `|` never serve as sides, corners, junctions or connectors: `/` appears only between two words (`R/W`, `put / get`, one space at most on each side) or opening or closing a path (`/sys/bus/`), `|` only inside a word (`a|b`), doubled (`||`) or in a comment, and both in reproduced source; the check flags the rest for reading, a spaced C expression included.
