...
9. [figure-check.legend] A figure's circled numbers match its legend, every legend site is reproduced by an excerpt and lies in the named function, and the Lifecycle table's rows are assignments to the named field by the named writer with no writer of a drawn field missing (figures.md [drawing.legend], worksheet.md [evidence.lifecycle]).
