...
2. [setup.runner] `python3 tools/qa/kg.py check docs/<dir>/<group>/<slug>.md` runs every `qa` requirement in guideline order and prints, per rule, its guideline location, the FAIL violations, the review work items and the notes, inventories and measurements; `--json` keeps every finding with its structured data, `--checklist` adds the manual and unexecuted requirements, `--first-pass` writes the run's counts per rule beside the worksheet once (worksheet.md [lint.first-pass]), `--only <rule-id>` selects rules and settles no page state, and zero findings is not a PASS. A full run ends with the page's state, WRITTEN or LINTED, from the page and QA digests in LINT. The companions: `kg where <slug>` locates a guideline, its check and its tests; `kg view` prints the page `--prose`, `--raw`, `--spans-visible` or by `--regions`; `kg table <page>` emits LINKS; `kg skim` and `kg excerpt` serve steps 12 and 1.
