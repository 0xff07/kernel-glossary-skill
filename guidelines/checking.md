...
2. [setup.runner] `python3 tools/qa/kg.py check docs/<dir>/<group>/<slug>.md` runs every `qa` requirement in guideline order and prints, per rule, its guideline location, the FAIL violations, the review work items and the notes, inventories and measurements; `--json` keeps every finding with its structured data, `--checklist` adds the manual and unexecuted requirements, `--only <rule-id>` selects rules and settles no page state, and zero findings is not a PASS. A full run ends with the page's state, WRITTEN or LINTED, from the page and QA digests in LINT. The companions: `kg where <slug>` locates a guideline, its check and its tests; `kg view` prints the page `--prose`, `--raw`, `--spans-visible` or by `--regions`; `kg table <page>` emits LINKS; `kg skim` and `kg excerpt` serve steps 12 and 1.
   - Inputs resolve by convention or by option: the tree is the checkout three directories above the skill (`<tree>/.claude/skills/kernel-glossary-skill`) or `--tree`; the worksheet is found under `progress/*/` at the page's mirrored path and accepted by its HEADER, or `--worksheet`; the baseline is the committed page at `HEAD` when it differs from the working copy, which the facts and arrangement rules compare against; the campaign spec is `campaigns/<dir>.md` for the page's top directory, so a campaign whose short name differs from that directory passes `--spec campaigns/<campaign>.md` at every check, the writer's QA runs and the check pass alike.
   - The resolver validates the source version, the cleanliness of the cited files (no uncommitted change in a file the page cites, every cited path tracked at the commit) and the worksheet's identity before any rule depends on them. A missing required input makes the rule INCOMPLETE and keeps its earlier findings; an engine error is reported on its own.
