...
12. [steps.skim] Skim. Read `python3 tools/qa/kg.py view <page> --skim`, the route, and every subsection's title, first and last sentence: it must read as the page's argument without the code, and `--load` prints the reading-load measurements to compare with the last pass.
...
3. [check-pass.reproduce] Re-run the figure sweep, the span closure against the worksheet's LINKS table (a discrepancy means the table was hand-edited or stale, and a sample of judgement cells is re-adjudicated), the excerpt and anchor checks in full, and the geometry check.
4. [check-pass.counts] Re-derive two or three load-bearing counts on a basis shaped differently from both the worksheet records.
