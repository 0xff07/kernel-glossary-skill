...
6. [sections.other-sources, qa] OTHER SOURCES holds material no model generated, kept for grounding a model and for a person checking the page by hand: the commits behind the mechanism with their mailing-list threads, and whatever a reader adds later to read beside the page. A model's contribution sits under a heading naming it, `### Added by <model>`, one block per writer pass: commit entries `[<commit subject> (commit <abbreviated sha>)](<trailer URL>)` with the URL taken byte for byte from a `Link:` trailer in `git log` or from semcode `dig`, never constructed, guessed or normalized, or one sentence saying that no commit of the path carries a trailer; a commit without a trailer is cited in prose by sha and subject and gets no entry. Everything outside those blocks is the reader's, in any form, added by hand. The section is the reader's shelf: an agent writes only inside its own block, and only when its brief or the user asks; it rewrites, reorders or removes nothing outside it, carries the section over verbatim when the page is rebuilt, and never fetches a link from it. The check verifies the blocks' entries against their trailers, lists the reader's lines without touching them, and fails a page whose committed version, once in this form, had a reader's line the page no longer carries.
...
1. [links.definition-line, qa] Every kernel symbol outside a fence (function, macro, struct, enum, typedef, argument forms included) is an Elixir link to its definition line at its first occurrence in each paragraph, table cell or list item, later occurrences in the same paragraph left as bare spans so the line stays readable, table cells included, and `struct` and `enum` keep their keyword everywhere. A call site, branch or field assignment is a location link whose text is `file:first-last` for the run of lines the prose describes, or `file:line` where one line is the site, the file named as [links.site-text] says and the URL anchored at the first line; prose that enumerates call sites carries one per site. One base, `https://elixir.bootlin.com/linux/<version>/source/`, across the page.
...
3. [links.bare-spans, qa] Left bare, in prose and table cells alike:
   - a symbol linked earlier in the same paragraph; C keywords and operators; locals, parameters and goto labels quoted from an excerpt;
   - literal and error values (`true`, `false`, `0`, `-EINVAL`, `NULL` as a value); literal strings a function prints or logs; a value or expression span, which links a constituent symbol where one exists;
   - `/proc`, `/sys`, sysctl and other path strings; Kconfig fragments and keywords (`=y`); tracepoint field names; commit hashes; `name(2)` notation when the entry point is linked elsewhere;
   - a wildcard family name (`VM_*`) when its members are linked nearby; a designated initializer's member name, whose citation is a location link at the initializer line; a symbol verified absent from the tree, whose absence the prose states;
   - a spec-defined or hardware name (`_PS0`, `SLP_EN`), which stays bare with a note saying so.
   Any other bare span is linked or recorded with its reason, a span in the descriptive text of a catalog bullet included: only the entry's own linked symbol belongs to the catalog, and the rest of the bullet is closed like prose.

4. [links.site-text, qa] A location link's text names its file by base name, `tb.c:130` or `tb.c:130-135`, not by its path from the tree root: the URL carries the path, the provenance of the excerpt beside the prose spells it out, and a table of sites stays narrow. Two exceptions keep the path: a page that cites two files sharing a base name, `drivers/thunderbolt/acpi.c` beside `arch/x86/pci/acpi.c`, writes both from the tree root, and a file without an extension, `Kconfig` or `Makefile`, keeps its path because the base name recurs across the tree. The check lists for reading a site text that carries a directory where no cited file shares its base name, and a bare base name where two cited files share it.

## Coverage of kernel constructs and driver examples [coverage]

1. [coverage.constructs] Cite every site that exhibits a behavior, with a location link at the mention and the sites the walkthrough explains as excerpts; when the set is too large, cite a representative spread and state the total. Cover:
   - the structs, enums and typedefs that hold the state and the helpers that allocate, initialize, read, modify and destroy them;
   - the lifecycle in full: allocation, initialization, teardown order, serializing locks, reference counting with the put that frees, state transitions and their drivers, notification mechanisms, deferred work and the ordering between them;
   - every hard-coded limit (timeout, retry count, error cap, buffer or queue size, poll or backoff interval) with its value, its defining symbol or literal and its file and line, reproduced where it governs a walked path.
   The last of the seven acceptance lines of checking.md [judgement], [judgement.constructs], records that this was done.
