...
Each entry maps a subsystem to its tag, output directory, primary kernel source paths, specification name(s), and the heading to use for section six. All entries live in this one file; a page-production task reads exactly the one entry it needs.

Entry schema (every entry carries these five fields):

- `tag`: the subsystem's short label, carried for reference; no pass reads it, a catalog row's tag column says requested or curated (campaign.md [planning.catalog]), and a commit title takes the campaign's short name (SKILL.md, policy 5)
...
- `section6_heading`: the heading to use for section six (REGISTERS, METHODS, PRIMITIVES, INTERFACES, or none, in which case the section is omitted), one value; a page family whose constructs take another of the four is named in a `note` line after the five fields, and the brief's "Section-six heading" line carries the page's value (campaign.md [briefs])
...
- note: the EC protocol pages use REGISTERS
