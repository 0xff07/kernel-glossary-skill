...
- Run `python3 <SKILL_DIR>/tools/qa/kg.py check <page>` once over the whole composed page before your first fix: the engine records that run as the first pass by itself (worksheet.md [lint.first-pass]). Then fix in batches: resolve everything a run lists, one edit per rule where its sites are alike, and run the check whole again after each batch, never after each fix and never over a partial page, since every run reads the whole page and the delta it prints (what is new since the last full run, the standing count per rule) makes a rerun cheap to read; `--only <rule-id>` is for a rule's standing list when the first run's is no longer at hand.
- Run the QA steps after the page is complete and fix what they find before reporting; run `python3 <SKILL_DIR>/tools/qa/kg.py table <page>`, which writes the LINKS table into your worksheet, and fill every kind / reason cell it reports to fill; persist the EVIDENCE and LINT sections into the worksheet, ending EVIDENCE with `page sha256: <digest>` of the page the evidence describes.
