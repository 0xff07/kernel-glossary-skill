...
- Run `python3 <SKILL_DIR>/tools/qa/kg.py check <page>` as the page grows, after each section you add, and once over the whole composed page before your first fix: the engine records that run as the first pass by itself (worksheet.md [lint.first-pass]). A full run prints what is new since the last full run and counts what stands per rule; resolve everything a run lists before the next full run, listing a rule's standing items with `--only <rule-id>` as you work through them.
